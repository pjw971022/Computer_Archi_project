...
if len(sys.argv) == 5:
    ...
    DataMemory = hexacode
    DataMemory = '{0:F<23107200}'.format(DataMemory)

    DataMemory = list(DataMemory)
else:
    DataMemory = '{0:F<23107200}'.format('')
...
class cache1():  # tag 싹 바꿔줘야함
    def __init__(self, index_len, offset_len):
        ...
        self.cache_data = [[0 for i in range(self.word_num)] for i in range(2 ** index_len)]
    ...
    def read_process(self, memnum, rd):
        ...
        memory_address = '{0:0>32}'.format(memory_address)
        ...
        # hit process
        if self.valid[index] == 1 and self.tag[index] == tag_val:
            self.hit += 1
            ...
            if memnum >= 0:
                # block swap in
                memnum = int(memnum / (2 ** (self.offset_len + 1)))
                memnum = memnum * (2 ** (self.offset_len + 1))
...
class cache2():
    def __init__(self, index_len, offset_len):  # 4-way associative
        ...
        # bits of cache2
        self.LRU = [[0, 1, 2, 3] for i in range(16)]
        self.valid = [0 for i in range(64)]
        self.dirty = [0 for i in range(64)]
        self.tag = [0 for i in range(64)]
        self.cache_data = [[[-1 for i in range(self.word_num)] for i in range(4)] for i in range(16)]
    ...
    def read_process(self, memnum, rd):
        ...
        memory_address = format(memnum, 'b')
        memory_address = '{0:0>32}'.format(memory_address)
        tag_bit = memory_address[0:22]
        set_bit = memory_address[22:26]
        offset_bit = memory_address[26:32]

        # bit's value setting
        set_num = copy.deepcopy(int(set_bit, 2))
        tag_val = copy.deepcopy(int(tag_bit, 2))
        word = int(int(offset_bit, 2) / 4)

        # hit or miss
        for i in range(4):
            if self.valid[set_num * 4 + i] and self.tag[set_num * 4 + i] == tag_val:
                self.hit += 1
                self.LRU[set_num].remove(i)
                self.LRU[set_num].append(i)
                if self.cache_data[set_num][i][word] != -1:
                    regVal[rd] = self.cache_data[set_num][i][word]
                else:
                    regVal[rd] = int('FFFFFFFF', 16)
        ...
        if memnum >= 0:
            ...
            self.valid[set_num * 4 + victim] = 1

            if self.dirty[set_num * 4 + victim] == 1:  # swap-out
                victim_mem = int(format(self.tag[set_num * 4 + victim], 'b') + set_bit + '000000', 2)
                victim_mem -= int('10000000', 16)
                victim_mem *= 2
                for w in range(self.word_num):
                    store_num = None
                    if self.cache_data[set_num][victim][w] != -1:
                        store_num = format(self.cache_data[set_num][victim][w], 'x')
                        store_num = '{0:0>8}'.format(store_num)
                    else:
                        store_num = 'FFFFFFFF'
                    for idx in range(8):
                        DataMemory[victim_mem + w * 8 + idx] = store_num[idx]

            self.tag[set_num * 4 + victim] = tag_val
            # block swap-in
            ...
            for i in range(self.word_num):
                ...
                if DataM != 'FFFFFFFF':
                    ...
                    self.cache_data[set_num][victim][i] = -1

            if self.cache_data[set_num][victim][word] != -1:
                regVal[rd] = self.cache_data[set_num][victim][word]
            else:
                regVal[rd] = int('FFFFFFFF', 16)
        else:
            print("memory address error")

    def write_process(self, memnum, rd):
